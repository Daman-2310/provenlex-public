// ProvenLex Obituary — forensic post-mortems of collapsed EU/global finance entities.
//
// Each obituary reconstructs what ProvenLex WOULD have flagged and when,
...
export interface Obituary {
  ...
  // ProvenLex prophecy trajectory (synthetic but plausible)
...
export const OBITUARIES: Obituary[] = [
...

  {
    ...
    what_genesis_would_have_seen:
      'Greensill exhibited the classic single-counterparty-concentration pattern. By 2019 a substantial fraction of receivables packaged into Credit Suisse-sold funds traced back to a small handful of entities, including the steel-and-energy group GFG Alliance (Sanjeev Gupta) and a network of related parties. Concentration this severe is a Pre-Crime indicator independent of any other signal: collapse risk scales with the failure probability of the single concentrated counterparty.\n\nThe second axis ProvenLex would have monitored is insurance-coverage continuity. Supply-chain finance programs are structurally insurance-dependent: when the insurer says no, the program ends. Public filings in 2019-2020 disclosed deteriorating insurance terms and increasingly bespoke insurer arrangements (single-syndicate, single-policy, single-broker). This narrowing would have driven the trajectory rating from HOLDING to RISING through 2020.\n\nThe third axis is political-cover overhang. When a former head of government joins as adviser and lobbies the Treasury for emergency Bank of England coverage, that itself is a signal: companies that can survive on commercial terms do not need political cover. Cameron\'s lobbying activity in late 2020 would have crossed ProvenLex pattern marker REGULATORY_LOBBYING_OVER_COMMERCIAL_TERMS.',
    signals: [
...

  {
    ...
    what_genesis_would_have_seen:
      'Archegos exhibits the pattern ProvenLex tracks as HIDDEN_LEVERAGE. Family offices are exempt from 13F holdings disclosure in the US; total-return swap positions are disclosed only to the prime broker counterparty. Yet leveraged concentration at this scale is observable indirectly: unusual single-name volume, large block prints, prime broker risk-weighted-asset disclosures that drift, and informal market chatter that always precedes public disclosure.\n\nA second ProvenLex axis is prior-violation history. Hwang had pleaded guilty in 2012 (as Tiger Asia) to wire fraud and insider trading in Chinese-bank stocks. ProvenLex weights prior conviction as a permanent base risk floor: re-entry to securities markets by a previously-sanctioned principal is itself a Pre-Crime indicator. From 2013 onward Hwang\'s subsequent activity would have been monitored at floor PCI 35+.\n\nThe third axis is prime-broker concentration. Archegos used at least six prime brokers in part to disguise the aggregate position. This is detectable from prime broker side: when multiple major dealers each hold a similar exposure to a single client whose other relationships are not disclosed, the SYSTEMIC_CONCENTRATION pattern triggers. ProvenLex would have called this collapse with high confidence and dated trajectory through 2020-Q4.',
    signals: [
...

  {
    ...
    what_genesis_would_have_seen:
      'SVB exhibits the pattern ProvenLex tracks as DEPOSIT_BASE_CONCENTRATION. A bank whose depositor base is structurally homogeneous (here: venture-capital-backed companies advised by a small group of VC firms) has a runnable deposit base in a way that diversified retail deposits do not. ProvenLex would have weighted SVB\'s base PCI at 35+ from at least 2020 on this fact alone.\n\nA second axis is the asset-liability duration mismatch in a rising-rate environment. SVB held a substantial AFS portfolio in long-duration MBS purchased at low yields. As the Fed raised rates from near-zero to 4.5% across 2022, the unrealized loss on that AFS portfolio grew to roughly $15 billion — nearly the bank\'s entire tangible common equity. This was disclosed quarterly. ProvenLex would have moved SVB to PCI 60+ through Q3 2022 on this basis alone.\n\nThe third axis is communication risk. In a bank with concentrated, sophisticated depositors who talk to each other constantly via social platforms (Twitter, Slack, group chats), the speed at which a run can develop is unprecedented. ProvenLex treats COMMUNICATION_VELOCITY as an amplifier — banks with concentrated, networked depositor bases face exponentially faster run dynamics than traditional retail banks. By early 2023, SVB scored at PCI 75+ and was on the Pre-Crime Watch list.',
    signals: [
