...
export async function sendMagicLink(email: string, link: string): Promise<{ delivered: boolean; via: 'resend' | 'console' }> {
  ...
  try {
    await client.emails.send({
      from: FROM,
      to: email,
      subject: 'Your ProvenLex sign-in link',
      html: `
<div style="font-family:system-ui,-apple-system,sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;color:#111">
  <h1 style="font-size:22px;font-weight:900;letter-spacing:.05em;color:#00aa55">PROVENLEX</h1>
  <p style="font-size:14px;color:#444">Tap the button to sign in to your ProvenLex dashboard. This link expires in 15 minutes.</p>
  ...
  <p style="font-size:10px;color:#aaa;text-transform:uppercase;letter-spacing:.1em">ProvenLex · RegTech AI · Luxembourg · CSSF-aligned</p>
</div>
      `,
      text: `Sign in to ProvenLex:\n\n${link}\n\nThis link expires in 15 minutes.`,
    })
